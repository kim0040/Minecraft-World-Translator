...
- 여러 LLM 공급자 전환 및 모델 목록 조회
- OpenAI 및 Anthropic 호환 Custom API 공급자 완벽 지원
- 번역 전 파일, 예상 토큰 및 시간 예측 안내 (Estimation)
- 보호가 필요한 무료 API를 위한 RPM / TPM 제한 설정
...
실행 전 `.env` 파일을 준비해도 된다.

```bash
cp .env.example .env
```

필요한 키만 채우면 되고, `.env`는 Git에 올라가지 않게 처리돼 있다.
...
위 값들은 셸 환경변수로 넣어도 되고, 로컬 `.env` 파일에 저장해도 된다.
